...
/* Array#[](start, len) and Array#[](range) -- returns a new IntArray.
   Out-of-range start returns NULL-equivalent (empty array, matching CRuby
   when start == length; CRuby returns nil when start > length but we'd
   need a separate nil path -- empty is safer). Negative start counts
   from the end. */
static sp_IntArray*sp_IntArray_slice(sp_IntArray*a,mrb_int start,mrb_int len){sp_IntArray*b=sp_IntArray_new();if(start<0)start+=a->len;if(start<0||start>a->len)return b;if(len<0)len=0;if(start+len>a->len)len=a->len-start;if(len==0)return b;if(len>b->cap){sp_gc_hdr*h=(sp_gc_hdr*)((char*)b-sizeof(sp_gc_hdr));sp_gc_bytes-=sizeof(mrb_int)*b->cap;h->size-=sizeof(mrb_int)*b->cap;b->cap=len;b->data=(mrb_int*)realloc(b->data,sizeof(mrb_int)*b->cap);h->size+=sizeof(mrb_int)*b->cap;sp_gc_bytes+=sizeof(mrb_int)*b->cap;}memcpy(b->data,a->data+a->start+start,sizeof(mrb_int)*len);b->len=len;return b;}
