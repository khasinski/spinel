  ...
  if (sp_streq(ty, "AndNode") || sp_streq(ty, "OrNode")) {
    ...
    #define EMIT_ARM(IS_RIGHT) do { \
      if (IS_RIGHT) { if (res == TY_POLY && comp_ntype(c, right) != TY_POLY) emit_boxed(c, right, b); else emit_expr(c, right, b); } \
               ...
               /* a nullable builtin hash/array pointer boxes like an object: \
                  NULL is nil (the AND kept-left arm only runs when the temp IS \
                  NULL). Only the hash kinds hash_box_cls() knows a builtin \
                  class for are covered here; other hash kinds fall through to \
                  the raw pointer temp below. Previously these builtin-hash \
                  temps fell to the raw pointer path, yielding a ternary with \
                  mismatched C operand types (sp_RbVal vs sp_PolyPolyHash *, \
                  e.g. doom's `picked && picked[idx]`). */ \
